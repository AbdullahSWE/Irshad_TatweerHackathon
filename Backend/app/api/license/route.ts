...
import { runLicenseRec } from '@/lib/llm';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const verifyResult = serverSession.verifyResult ?? {
      status: 'not_found' as const,
      whatToConfirm: 'license fee and requirements',
    ...
    const licenseResult = await runLicenseRec(syntheticSession, verifyResult);

    updateServerSession(sessionId, { licenseResult });

    return NextResponse.json({ license: licenseResult, nextStage: 'banking' });
  } catch (err) {
