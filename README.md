...
**A voice-controlled Arabic/English business guide that feels like a normal conversation and uses a curated UAE license and bank database to create your tailored business action plan.**
...
<sub>Demo: spoken idea → adaptive questions → grounded roadmap.</sub>
...
| **Built** | Working SwiftUI app, voice input/output, adaptive cards, final plan screen, and a sourced UAE knowledge base. |
...
| **How to verify** | Detailed instructions in Section IX. |
...
Irshad is a voice-first iOS guide. The founder speaks an idea. The app asks a small number of adaptive questions. Irshad fills a structured business profile. Once enough information is known, Irshad produces a plan with:
...
| 3 | The project is deployable with light infrastructure. | A simple Apple TestFlight deployment. No GPU or heavy database required for the prototype. |
...
|---|---|
| `knowledge.json` | 7 authorities, 6 licence types, 6 banks, 5 loan products, 10 funds/programmes, 11 business archetypes |
...
| TestFlight | Shared through Apple TestFlight or run locally through Xcode. Instructions in Section IX. |
...
| **Knowledge** | `knowledge.json`, source URLs, authorities, licences, banks, programmes, archetypes. |
...
| It needs internet | LLM runs remotely. | Add cached KB and offline draft mode. |
